...
SET @RepStart = '10/01/2016';
SET @RepEnd = '10/31/2016';
...
'SELECT COUNT(FLOW_TYPE) AS "UMRs sent (REN FLOW_LOG)"
       FROM SHIP_GAS_CUST.FLOW_LOG
       WHERE FLOW_TYPE LIKE ''''UMR%''''
       AND PROCESS_TIMESTAMP >= TO_DATE(''''' + REPLACE(CONVERT(CHAR(9), @RepStart, 6),' ','-') + ''''',''''DD-MON-YY'''') 
       AND PROCESS_TIMESTAMP <= TO_DATE(''''' + REPLACE(CONVERT(CHAR(9), @RepEnd, 6),' ','-') + ''''',''''DD-MON-YY'''')';
--SELECT @SqlQuery

SET @OpenQuery = 'SELECT * FROM OPENQUERY(AFMSELECLIVEP4,' + '''' + @SqlQuery + '''' + ')';
--SELECT @OpenQuery
EXEC (@OpenQuery);
...
SET @SqlQuery = 
'SELECT COUNT(FLOW_TYPE) AS "UMRs sent (CNG FLOW_LOG)"
       FROM AFMS_GAS_CUST.FLOW_LOG
       WHERE FLOW_TYPE LIKE ''''UMR%''''
       AND PROCESS_TIMESTAMP >= TO_DATE(''''' + REPLACE(CONVERT(CHAR(9), @RepStart, 6),' ','-') + ''''',''''DD-MON-YY'''') 
       AND PROCESS_TIMESTAMP <= TO_DATE(''''' + REPLACE(CONVERT(CHAR(9), @RepEnd, 6),' ','-') + ''''',''''DD-MON-YY'''')';
--SELECT @SqlQuery
...
SET @SqlQuery = 
'SELECT COUNT(FLOW_TYPE) AS "URSs recd (CNG FLOW_LOG)"
       FROM AFMS_GAS_CUST.FLOW_LOG
       WHERE FLOW_TYPE LIKE ''''URS%''''
       AND PROCESS_TIMESTAMP >= TO_DATE(''''' + REPLACE(CONVERT(CHAR(9), @RepStart, 6),' ','-') + ''''',''''DD-MON-YY'''') 
       AND PROCESS_TIMESTAMP <= TO_DATE(''''' + REPLACE(CONVERT(CHAR(9), @RepEnd, 6),' ','-') + ''''',''''DD-MON-YY'''')';
...
FROM gas.combined_mprn mp
	...
	INNER JOIN [AFMSELECLIVEP4]..[SHIP_GAS_CUST].[METER_READ] sr
	--INNER JOIN OPENQUERY(AFMSELECLIVEP4, 'SELECT * FROM SHIP_GAS_CUST.METER_READ') AS sr -- slower!!
		ON mp.SHIPPER = 'REN'
		AND ( sr.LAST_UPD >= @RepStart AND sr.LAST_UPD <= @RepEnd )
		AND me.METER_PK - 90000000 = sr.METER_FK
	LEFT JOIN [AFMSELECLIVEP4]..[SHIP_GAS_CUST].[METER_READ_REJ_REASON] rj
...
FROM gas.combined_mprn mp
	...
	LEFT JOIN gas.combined_agent ag
		ON mp.MPRN_PK = ag.MPRN_FK 
	INNER JOIN [AFMSELECLIVEP4]..[AFMS_GAS_CUST].[METER_READ] sr
	--INNER JOIN OPENQUERY(AFMSELECLIVEP4, 'SELECT * FROM AFMS_GAS_CUST.METER_READ') AS sr -- slower!!
		ON mp.SHIPPER = 'CNG'
		AND ( sr.LAST_UPD >= @RepStart AND sr.LAST_UPD <= @RepEnd )
		AND me.METER_PK = sr.METER_FK
	LEFT JOIN [AFMSELECLIVEP4]..[AFMS_GAS_CUST].[METER_READ_REJ_REASON] rj
		ON sr.METER_READ_PK = rj.METER_READ_FK
WHERE sr.FLOW_RECEIVED IN ( 'URS_U10', 'URS_U02' )
), cng210 AS 
-- return all 210s where read last updated date is within report start date - 14 days and report end date
...
FROM PivotData
PIVOT(COUNT(ReadNum) 
FOR ReadStatus IN ( "ACCEPTED" , "FAILED" )) AS PivotResult
ORDER BY MRA ASC;
