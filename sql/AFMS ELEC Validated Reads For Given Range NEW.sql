...
-- EDIT 20190203 - Now uses new SQL server
...
FROM dbo.mpan mp
	...
	LEFT JOIN [P-OVL6-AFMSQL-1].[CUSTOMER].dbo.[METER_REG_READING] rr
		ON mr.METER_REG_PK = rr.METER_REG_FK
		AND ( rr.DATE_RECEIVED >= @RepStart AND rr.DATE_RECEIVED <= @RepEnd )
-- *** EDIT 13/06/2016 
	LEFT JOIN OPENQUERY ([P-OVL6-AFMSQL-1], 'SELECT a.MPAN_LNK, a.AGENT_ID, a.AGENT_ROLE_CODE, a.AGENT_EFD, a.AGENT_ETD
													FROM CUSTOMER.dbo.AGENT_HISTORY a
