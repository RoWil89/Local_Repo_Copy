...
Purpose:To select MPANS which are not settling which can be raised for a must read visit
...
08: Returns a table with address information and adds a rule to identify incomplete address details
08: Returns request details
09: Returns register details
11: Final request output
...
,CASE
WHEN E.ErrorType = 'Rejected Reads' AND Meter_Type LIKE 'S%' THEN 'Remove'
ELSE 'Keep'
END AS 'SMART Rej Read Filter'
...
AND SVCCs_Associated_With_D0004 NOT IN ('04', '05', '06', '07', '08', '11', '14', '15', '16', '17', '18', '23', '24', '25', '28', '31', '32', '33', '34', '35', '41', '58')
AND DC = 'SIEM'
AND [PSR Indicator] = 0
AND D0019_Status != 'Investigate'
AND [Days Since Last Read] >= '180'
AND [SMART Rej Read Filter] = 'Keep'


ORDER BY [Last Read Date] ASC, Total_EAC desc
...
SELECT *
	...
	,CASE
		WHEN Address = '' THEN 1
		ELSE 0
		END AS 'Incomplete address indicator'

INTO #Requests

FROM DatSup.mds.Requests

WHERE MPAN IN (SELECT MPAN FROM #SELECTION)
...
SELECT 
	 ...
	 '' AS 'RequestReference'
	,'' AS 'RequestGenerationDateTime'
	,MPAN
	,Utility
	,'Must Read' AS 'JobType'
	,MeterSerialNo
	,'' AS 'NoOfDials'
	,'' AS 'AppointmentDate'
	,'' AS 'AppointmentTime'
...
WHERE [Incomplete address indicator] = 0
...
FROM DatSup.mds.Registers

WHERE MPAN IN (SELECT MPAN FROM #Requests WHERE [Incomplete address indicator] = 0)
...
SELECT DISTINCT MPAN, Total_EAC, [Last Read Date] FROM #SELECTION WHERE MPAN IN (SELECT MPAN FROM #Requests where [Incomplete address indicator] = 0)