/*\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
Title:			Elec - Last Valid Meter Read Date
VERSION:		1.0
Author:			Craig Wilkins
Server(s):		uh-gendb-01
Dastabase(s):	AFMSLocal
Date Created:   2021.03.10
Purpose:		To show the last validated read date for each live elec MPAN
Sections:	
...
ON ME.METER_PK = MR.METER_FK
	...
	--AND MR.ETD_ID IS NULL -- Only look at live Reg IDs
...
SELECT DISTINCT
	...
	,DA
	,DC
	,FIRST_VALUE([Reading Date]) OVER (PARTITION BY LatestValidRead.MPAN ORDER BY DATE_RECEIVED DESC) AS 'Reading Date'
	,[BSC Validation Status]
	,FIRST_VALUE([Reading Type]) OVER (PARTITION BY LatestValidRead.MPAN ORDER BY DATE_RECEIVED DESC) AS 'Reading Type'
	,FIRST_VALUE(FLOW_RECEIVED) OVER (PARTITION BY LatestValidRead.MPAN ORDER BY DATE_RECEIVED DESC) AS 'FLOW_RECEIVED'
	,FIRST_VALUE(Bracket) OVER (PARTITION BY LatestValidRead.MPAN ORDER BY DATE_RECEIVED DESC) AS 'Bracket'
	,FIRST_VALUE([Bracket Index]) OVER (PARTITION BY LatestValidRead.MPAN ORDER BY DATE_RECEIVED DESC) AS 'Bracket Index'
	,MAX(DATE_RECEIVED) OVER (PARTITION BY LatestValidRead.MPAN) AS 'DATE_RECEIVED' 
	 ,SUM(#Latest_Valid_EAC.EAC) OVER (PARTITION BY LatestValidRead.MPAN) AS 'Total_EAC'
	 ,MAX(#Latest_Valid_EAC.EAC_EFD) OVER (PARTITION BY LatestValidRead.MPAN) AS 'Max_EAC_EFD' 
